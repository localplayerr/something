	...
	while task.wait() do
		task.spawn(function()
			...
			for _ , v in game.Workspace.PlotItems["Plot"..SlotSelect["plar"].Text]:GetChildren() do
				if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") then table.insert(dums,v.Humanoid) elseif table.find(dums,v) then table.remove(dums,v) end
			end
